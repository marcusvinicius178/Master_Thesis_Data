...
# DEVIATION YAW ANGLE = HEADING
    ...
    def deviation_lists_autoware_heading(self, df_pose_orientation_h, df_pose_orientation_final_wp_h,time_stamp_array_final_wp_h, time_stamp_array_current_pose_h, planner_name_h):
        self.df_pose_orientation_h = df_pose_orientation_h
        self.df_pose_orientation_final_wp_h = df_pose_orientation_final_wp_h
        self.time_stamp_array_final_wp_h = time_stamp_array_final_wp_h
        self.time_stamp_array_current_pose_h = time_stamp_array_current_pose_h
        
        self.planner_name_h = planner_name_h


        self.final_list_final_wp_h = []
        for i in range(len(self.time_stamp_array_final_wp_h)):
            
            time_value = self.time_stamp_array_final_wp_h.iloc[i]
            human_readable = datetime.fromtimestamp(time_value // 1e9)
            string_human_readable = human_readable.strftime(('%Y-%m-%d %H:%M:%S')) 
            ...
            self.final_list_final_wp_h.append(new_s)
        
        
        self.final_list_current_pose_h = []
        for i in range(len(self.time_stamp_array_current_pose_h)):
            
            time_value = self.time_stamp_array_current_pose_h.iloc[i]
            human_readable = datetime.fromtimestamp(time_value // 1e9)
            string_human_readable = human_readable.strftime(('%Y-%m-%d %H:%M:%S')) 
            ...
            self.final_list_current_pose_h.append(new_s)
        
        
      #DATAFRAME of final_waypoints topic Data  
        self.Redo_Df_final_waypoints_h = pd.DataFrame({'TimeStamp':self.final_list_final_wp_h})
        self.append_df_final_wp_h = self.df_pose_orientation_final_wp_h.join(self.Redo_Df_final_waypoints_h)
    #DATAFRAME of current_pose topic DATA
        self.Redo_Df_current_pose_h = pd.DataFrame({'TimeStamp':self.final_list_current_pose_h})
        self.append_df_h = self.df_pose_orientation_h.join(self.Redo_Df_current_pose_h)
    #Required to synchronize these DataFrames above on TimeSTamp column in order to subtract the corresponding waypoints on time        
        
        
        self.df_synchronized_pose_orientation_h = self.append_df_final_wp_h.merge(self.append_df_h, on="TimeStamp", copy=False)
        self.df_synchronized_pose_orientation_h.to_csv('Visualize_Merged_Df.csv')
        #-------------------------EXTRACT ARRAYS x,y,z for each DATAFRAME SOURCE-----------------------
        
        
        self.Deviation_x_List_h = []
        self.Deviation_y_List_h = []
        self.Deviation_z_List_h = []
        
        for dev in range(len(self.df_synchronized_pose_orientation_h)-1):
            #x_deviation = self.df_synchronized_pose_orientation['field.waypoints0.pose.pose.position.x'].iloc[dev] - self.df_synchronized_pose_orientation['field.pose.position.x'].iloc[dev]
            #y_deviation = self.df_synchronized_pose_orientation['field.waypoints0.pose.pose.position.y'].iloc[dev] - self.df_synchronized_pose_orientation['field.pose.position.y'].iloc[dev]
            x_deviation = self.df_synchronized_pose_orientation_h['field.pose.position.x'].iloc[dev] - self.df_synchronized_pose_orientation_h['field.waypoints0.pose.pose.position.x'].iloc[dev]
            y_deviation = self.df_synchronized_pose_orientation_h['field.pose.position.y'].iloc[dev] - self.df_synchronized_pose_orientation_h['field.waypoints0.pose.pose.position.y'].iloc[dev]
            #To do the calculation for z it is required to convert quaternion to Euler(yaw) from final waypoints.....as it was done during rosbag extraction for current_pose
            #z_deviation = self.df_synchronized_pose_orientation['field.waypoints0.pose.pose.orientation.z.yaw'].iloc[dev] - self.df_synchronized_pose_orientation['yaw'].iloc[dev]
            z_deviation =   abs(self.df_synchronized_pose_orientation_h['yaw'].iloc[dev]) - abs(self.df_synchronized_pose_orientation_h['field.waypoints0.pose.pose.orientation.z.yaw'].iloc[dev])

            
            self.Deviation_x_List_h.append(x_deviation)
            self.Deviation_y_List_h.append(y_deviation)
            self.Deviation_z_List_h.append(z_deviation)
      
        
        
                    
        self.Deviation_Df_h = pd.DataFrame({'x_dev':self.Deviation_x_List_h, 'y_dev':self.Deviation_y_List_h, 'yaw_dev':self.Deviation_z_List_h})    
        #DIFFERENCE ON FUNCTIONS START BELOW
        
        self.pose_x_list_h = self.df_synchronized_pose_orientation_h['field.waypoints0.pose.pose.position.x']
        #get relative position in x    
        self.relative_position_x_h = []
                #self.abs_relative_pose_x = []
        for rel_x in range(len(self.pose_x_list_h)-1):
            rel_value = self.pose_x_list_h[rel_x] - self.pose_x_list_h[0]
            self.relative_position_x_h.append(rel_value)
            
        
        self.relative_position_x_h = [-i for i in self.relative_position_x_h]



        self.Deviation_heading_h = self.Deviation_Df_h['yaw_dev']     
        self.Deviation_heading_x_path_h = pd.DataFrame({'Path Position':self.relative_position_x_h,
                                                      'Heading Deviation':self.Deviation_heading_h})
        
        self.df_difference_h = self.Deviation_heading_x_path_h['Path Position'].diff()
        self.df_abs_difference_h = self.df_difference_h.abs()
    
        self.filter_redundant_origin_pose_x_yaw_h = self.Deviation_heading_x_path_h[self.df_abs_difference_h>0.01]
        #Add standard last position do Yaw Df
        self.last_pose_to_yaw_df_h = pd.DataFrame({'Path Position':[80], 'Heading Deviation':[None]})
        self.add_standart_last_pose_yaw_h = self.filter_redundant_origin_pose_x_yaw_h.append(self.last_pose_to_yaw_df_h) 
        self.add_standart_last_pose_yaw_sort_h = self.add_standart_last_pose_yaw_h.sort_values(by=['Path Position'])
        self.add_standart_last_pose_yaw_sort_h.reset_index(drop=True, inplace=True)
        self.add_standart_last_pose_yaw_sort_interpolate_h = self.add_standart_last_pose_yaw_sort_h.interpolate()
        #Removing the additional Path >80
        self.filter_additional_path_x_yaw_h = self.add_standart_last_pose_yaw_sort_interpolate_h[self.add_standart_last_pose_yaw_sort_interpolate_h['Path Position']<80.5]
        #Recovering origin DF
        self.origin_df_recover_path_x_yaw_h = [self.Deviation_heading_x_path_h.iloc[0]['Path Position']]
        self.origin_df_recover_deviation_yaw_h = [self.Deviation_heading_x_path_h.iloc[0]['Heading Deviation']]
        
        self.origin_yaw_recover_Df_h = pd.DataFrame({'Path Position':self.origin_df_recover_path_x_yaw_h,
                                             'Heading Deviation':self.origin_df_recover_deviation_yaw_h})
        
        self.heading_deviation_autoware_h = self.origin_yaw_recover_Df_h.append(self.filter_additional_path_x_yaw_h)
        self.heading_deviation_autoware_column_rename_h = self.heading_deviation_autoware_h.rename(columns={'Heading Deviation': self.planner_name_h})
...
Astar.deviation_lists_autoware_heading(df_pose_orientation_h=Astar_df_pose_orientation, df_pose_orientation_final_wp_h=Astar_df_pose_orientation_final_wp,
                   time_stamp_array_final_wp_h=Astar_time_stamp_array_final_wp, time_stamp_array_current_pose_h=Astar_time_stamp_array_current_pose, planner_name_h='Astar')
...
Lattice.deviation_lists_autoware_heading(df_pose_orientation_h=Lattice_df_pose_orientation, df_pose_orientation_final_wp_h=Lattice_df_pose_orientation_final_wp,
                   time_stamp_array_final_wp_h=Lattice_time_stamp_array_final_wp, time_stamp_array_current_pose_h=Lattice_time_stamp_array_current_pose, planner_name_h='Lattice')
...
Op.deviation_lists_autoware_heading(df_pose_orientation_h=Op_df_pose_orientation, df_pose_orientation_final_wp_h=Op_df_pose_orientation_final_wp,
                   time_stamp_array_final_wp_h=Op_time_stamp_array_final_wp, time_stamp_array_current_pose_h=Op_time_stamp_array_current_pose, planner_name_h='Open Planner')
...
#Open Planner
ax = Astar.xy_deviation_autoware_column_rename.plot(kind='line', x='Path Position', linewidth=1.25, color= 'Black', ax=axes[0], legend=False)
Lattice.xy_deviation_autoware_column_rename.plot(ax= ax, kind='line', x='Path Position', linewidth=1, color= 'Red', legend=False)
Op.xy_deviation_autoware_column_rename.plot(ax = ax, kind='line', x='Path Position', y= Op.planner_name, linewidth=1, color= 'Green', legend= False)
...
ax = Astar.heading_deviation_autoware_column_rename_h.plot(kind='line', x='Path Position', linewidth=2, color= 'Black', legend=True,  ax=axes[1])
Lattice.heading_deviation_autoware_column_rename_h.plot(ax= ax, kind='line', x='Path Position', linewidth=1, color= 'Red', legend=True)
Op.heading_deviation_autoware_column_rename_h.plot(ax= ax, kind='line', x='Path Position', y= Op.planner_name, linewidth=1, color= 'Green', legend=True)
