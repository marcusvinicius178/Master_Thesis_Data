...
Public_Road_Df_pose = pd.read_csv(r'Public_Road_localization_Static_Truck.csv')
...
class Deviation_xyz():
    ...
    def deviation_lists_autoware(self, df_pose_orientation, df_pose_orientation_final_wp,time_stamp_array_final_wp, time_stamp_array_current_pose, planner_name):
        ...
        for i in range(len(self.time_stamp_array_final_wp)):
            ...
            self.final_list_final_wp.append(human_time_float)
        ...
        for i in range(len(self.time_stamp_array_current_pose)):
            ...
            human_time_float = datetime.strptime(str_human_time, '%Y-%m-%d %H:%M:%S.%f')
            #print(human_time_float)
            self.final_list_current_pose.append(human_time_float)
...
class Deviation_xyz_Apollo(): 
    ...
    def deviation_lists_apollo(self,lateral_deviation_array, heading_deviation_array, planner_name):
        self.planner_name = planner_name
        
        self.lateral_deviation_array = lateral_deviation_array
        self.heading_deviation_array = heading_deviation_array
        
        linspace_interval_x = len(self.heading_deviation_array)  #80 = length of path
        
        self.path_x = np.linspace(0, 80, linspace_interval_x)
        self.path_x_list = self.path_x.tolist()
        
        self.xy_Lateral_Deviation_Df = pd.DataFrame({'Path Position':self.path_x_list,
                                                     'Lateral Deviation':self.lateral_deviation_array}) 
        
        self.heading_deviation_Df = pd.DataFrame({'Path Position':self.path_x_list,
                                                     'Heading Deviation':self.heading_deviation_array}) 
        
        self.heading_deviation_Df_column_rename = self.heading_deviation_Df.rename(columns={'Heading Deviation':self.planner_name})
        #self.xy_Lateral_Deviation_Df.plot('Path Position', 'Lateral Deviation')
        
        #self.heading_deviation_Df.plot('Path Position', 'Heading Deviation')
...
Public_Road.deviation_lists_apollo(lateral_deviation_array = Apollo_lateral_deviation_array, heading_deviation_array = Apollo_heading_deviation_array, planner_name = 'Public Road')
...
#Open Planner
ax = Op.xy_deviation_autoware_column_rename.plot(kind='line', x='Path Position', y= Op.planner_name, linewidth=1, color= 'Green', legend=True, ax=axes[0])
Astar.xy_deviation_autoware_column_rename.plot(ax= ax, kind='line', x='Path Position', linewidth=1, color= 'Black', legend=True)
Lattice.xy_deviation_autoware_column_rename.plot(ax= ax, kind='line', x='Path Position', linewidth=1, color= 'Red', legend=True)
...
Public_Road.heading_deviation_Df_column_rename.plot(ax = ax, kind='line', x='Path Position', linewidth =1, color= 'Blue', legend=True)
