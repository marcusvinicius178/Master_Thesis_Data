...
#Get the approach distance for each waypoint of trajectory: Distance of AV to obstaclein X axis 
    ...
    #THE DISTANCE BELOW IS WHEN X=0, IN OTHER WORDS, WHEN THE CAR IS PARALLELL =EQUAL TO OBSTACLE POSITION IN X
        ...
        self.text = str('ΔYmin = '+str(self.Delta_xy_str))
...
Scratch.text = str('ΔYmin = '+str(Public_Road.Delta_xy_str))
#plt.text(0.1,0.9,'ΔYmin ='+ self.Delta_xy_str,rotation=0)
#plt.show()
left, width = .0, 1
bottom, height = .0, 1
...
Astar.text = str('ΔXYmin = '+str(Astar.Delta_xy_str))
...
Lattice.text = str('ΔXYmin = '+str(Lattice.Delta_xy_str))
...
Op.text = str('ΔXYmin = '+str(Op.Delta_xy_str))
...
Public_Road.text = str('ΔXYmin = '+str(Public_Road.Delta_xy_str))
