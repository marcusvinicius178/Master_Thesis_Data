...
df_vel_1 = pd.read_csv(r'Lattice_current_pose_velocity.csv')
df_pose_1 = pd.read_csv(r'Lattice_current_pose_velocity.csv')

df_yaw_angle_aux = pd.read_csv(r'Lattice_converted_quaternions_to_Euler.csv')
...
#To plot the obstacle_Distance_Position
pose_x_array_Lattice = pose_x_array
pose_y_array_Lattice = pose_y_array
...
Lattice_cum_time = np.cumsum(progressive_time_list)

#-----------------------TRIP TOTAL TIME CALCULATION------------------------------------------------
Trip_seconds_Lattice = sum(final_list_delta_seconds)
Lattice_trip_seconds = np.linspace(0,Trip_seconds_Lattice,len(time_stamp_array))
#-------------------------------------------------------------------------------------------------
...
Lattice_accel_xy = acceleration_x_list
Lattice_accel_xy.insert(0,0) #The first velocity  value there is not acceleration, so a=0
...
Lattice_Steering_yaw_List = Steering_yaw_list
Lattice_Steering_yaw_List.insert(0,0)
...
Cum_delta_x_pose_abs_list_Lattice = sum(delta_x_pose_abs_list)
...
Lattice_cummulative_xy_position = Cummulative_delta_x_pose_list
origin = 0
Lattice_cum_xy_position = [origin] + Lattice_cummulative_xy_position
...
Total_path_x_traveled_Lattice = standard_x_goal_waypoint_planners #sum(delta_x_pose_abs_list)
Total_path_y_traveled_Lattice = sum(delta_y_pose_abs_list)
Total_path_xy_traveled_Lattice = math.sqrt(Total_path_x_traveled_Lattice**2 + Total_path_y_traveled_Lattice**2)


Cummulative_pose_xy_Lattice = delta_pose_xy_module_list.sum()


Filter_by_cum_x_Df_Lattice = pd.DataFrame({'Cummulative X': Cummulative_delta_x_pose_list,
...
Acceleration_xy_df_Lattice = Acceleration_rejoin_delta_df
...
Mean_Lattice_Acceleration_power = Power_Acceleration_df_hp.mean()
Median_Lattice_Acceleration_power = Power_Acceleration_df_hp.median()
#deviation
std_dev_Lattice_acceleration = stdev(Power_Acceleration_df_hp)
variance_Lattice_acceleration = variance(Power_Acceleration_df_hp)
...
CV_Accel_Power = std_dev_Lattice_acceleration / Mean_Lattice_Acceleration_power

#Mean/Average?/Median Brake Power (BHP)
Mean_Lattice_brake_power = Power_Brake_df_hp.mean()
Median_Lattice_brake_power = Power_Brake_df_hp.median()

std_dev_Lattice_brake = stdev(Power_Brake_df_hp)
variance_Lattice_acceleration = variance(Power_Brake_df_hp)
#CV variation coefficient = Standard deviation / mean. If CV < 1 indicates a low variation, however for non-normal distribution the standard deviation does not mean anything, also the CV
#It just says how much your data is spread from the mean...but does not say if it is wrong!
CV_Brake_Power = std_dev_Lattice_brake / Mean_Lattice_brake_power
...
#NEW BLOCK CUM TIME
Cummulative_time_velocity_df = Joined_velocity_xy_df["Delta Time"].cumsum()
Aux_cumtime_list = Cummulative_time_velocity_df.to_list()
Cummulative_time_velocity_df_new = pd.DataFrame({"Cummulative Time":Cummulative_time_velocity_df})
Join_cum_time_Lattice_df = pd.merge(Joined_velocity_xy_df,Cummulative_time_velocity_df_new, left_index=True, right_index=True)
Total_Velocity_xy_df = Join_cum_time_Lattice_df[['Velocity X', 'Velocity Y']]
Total_vel_xy_Pow = Total_Velocity_xy_df.pow(2)
Total_vel_xy_sum = Total_vel_xy_Pow['Velocity X'] + Total_vel_xy_Pow['Velocity Y']
Total_vel_xy_Resultant = Total_vel_xy_sum**(1/2)
aux_velxy_list =  Total_vel_xy_Resultant.to_list()
Total_vel_xy_Res_df = pd.DataFrame({'Velocity XY':aux_velxy_list})
Total_vel_xy_Cum_Time = pd.concat([Total_vel_xy_Res_df, Join_cum_time_Lattice_df['Cummulative Time']], axis=1)
...
get_back_energy_complete_df = pd.concat([delta_mechanic_column_rename, Delta_Time_df], axis=1)

#test
Q1_energy = get_back_energy_complete_df[["Mechanic Energy (ΔE)"]].quantile(0.25)
Q3_energy = get_back_energy_complete_df[["Mechanic Energy (ΔE)"]].quantile(0.75)
IQR_energy = Q3_energy - Q1_energy
#print(IQR)
Outliers_fst_quartile_Q1_energy = get_back_energy_complete_df[["Mechanic Energy (ΔE)"]] < (Q1 - 1.5 * IQR_energy) 
Outliers_4th_quartile_Q1_energy = get_back_energy_complete_df[["Mechanic Energy (ΔE)"]] > (Q3 + 1.5 * IQR_energy)

remove_Outliers_velocity_energy_IQR_method = get_back_energy_complete_df[~((get_back_energy_complete_df < (Q1_energy - 1.5 * IQR_energy)) |(get_back_energy_complete_df > (Q3_energy + 1.5 * IQR_energy))).any(axis=1)]
...
Power_Consumption_Watts_df = Filtered_Parameters_velocity_energy_Df["Mechanic Energy (ΔE)"] / Filtered_Parameters_velocity_energy_Df["Delta Time"]
Power_Consumption_Horsepower_df = Power_Consumption_Watts_df / 745.699872

Delta_cumulative_time = Filtered_Parameters_velocity_energy_Df["Delta Time"].cumsum()
...
aux_list_energy_x = New_Delta_cumulative_time['Cummulative Time Variation (ΔT)'].to_list()
Delta_Time_x = pd.DataFrame({"Cummulative Time Variation (ΔT)":aux_list_energy_x})
aux_list_energy_y = Filtered_Parameters_velocity_energy_Df["Mechanic Energy (ΔE)"].to_list()
Energy_Df_y = pd.DataFrame({"Mechanic Energy (ΔE)":aux_list_energy_y})
plot_df = pd.concat([Energy_Df_y, Delta_Time_x ], axis=1)
plot_reset_index = plot_df.reset_index()


#Plot the curve the best fits: Line, polynomial or other curves

x = plot_df["Cummulative Time Variation (ΔT)"]                   
y = plot_df["Mechanic Energy (ΔE)"] 
...
#FOR THE LATTICE PLANNER IT WAS NOT POSSIBLE FO FIND A EQUATION (Linear, or Polynomial)
...
Lattice_Power_Consumption_Derivative_Values_Sum = Instantaneous_Hp_Df
...
#Final_Power_Lattice_mean_plus_stdev = Mean_Power_Consumption + standard_deviation_Power_Consumption


#The power through line adjustment is not working for lattice. Therefore I will take directly
#The value of division ΔE)/ ΔT....and the average
Lattice_Power_Consumption_aux_watts = y/x


Lattice_Power_Consumption = Lattice_Power_Consumption_aux_watts.mean() / 745.699872