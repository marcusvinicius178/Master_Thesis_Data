...
df_vel_1 = pd.read_csv(r'Astar_current_pose_velocity.csv')
df_pose_1 = pd.read_csv(r'Astar_current_pose_velocity.csv')

df_yaw_angle_aux = pd.read_csv(r'Astar_converted_quaternions_to_Euler.csv')
...
df_difference = df_synchronize_yaw_to_current_pose_velocity['field.pose.position.x'].diff()
df_abs_difference = df_difference.abs()

#Removing the time of modules activation in Autoware.AI Runtime Manager
filtered_real_car_journey = df_synchronize_yaw_to_current_pose_velocity[df_abs_difference>0.05]
...
#To plot obstacle distance
pose_x_array_Astar = pose_x_array
pose_y_array_Astar = pose_y_array
...
Astar_cum_time = np.cumsum(progressive_time_list)

#-----------------------TRIP TOTAL TIME CALCULATION------------------------------------------------
Trip_seconds_Astar = sum(final_list_delta_seconds)
Astar_trip_seconds = np.linspace(0,Trip_seconds_Astar,len(time_stamp_array))
#-------------------------------------------------------------------------------------------------
...
Astar_accel_xy = acceleration_x_list
Astar_accel_xy.insert(0,0)
...
Astar_Steering_yaw_List = Steering_yaw_list
Astar_Steering_yaw_List.insert(0,0)
...
Sum_delta_x_pose_abs_list_Astar = sum(delta_x_pose_abs_list)
...
Astar_cummulative_xy_position = Cummulative_delta_x_pose_list
origin = 0
Astar_cum_xy_position = [origin] + Astar_cummulative_xy_position
...
Total_path_x_traveled_Astar = standard_x_goal_waypoint_planners #sum(delta_x_pose_abs_list)
Total_path_y_traveled_Astar = sum(delta_y_pose_abs_list)
Total_path_xy_traveled_Astar = math.sqrt(Total_path_x_traveled_Astar**2 + Total_path_y_traveled_Astar**2)

Cummulative_pose_xy_Astar = delta_pose_xy_module_list.sum()


Filter_by_cum_x_Df_Astar = pd.DataFrame({'Cummulative X': Cummulative_delta_x_pose_list,
...
Acceleration_xy_df_Astar = Acceleration_rejoin_delta_df
...
Mean_Astar_Acceleration_power = Power_Acceleration_df_hp.mean()
Median_Astar_Acceleration_power = Power_Acceleration_df_hp.median()
#deviation
std_dev_Astar_acceleration = stdev(Power_Acceleration_df_hp)
variance_Astar_acceleration = variance(Power_Acceleration_df_hp)
...
CV_Accel_Power = std_dev_Astar_acceleration / Mean_Astar_Acceleration_power

#Mean/Average?/Median Brake Power (BHP)
Mean_Astar_brake_power = Power_Brake_df_hp.mean()
Median_Astar_brake_power = Power_Brake_df_hp.median()

std_dev_Astar_brake = stdev(Power_Brake_df_hp)
variance_Astar_acceleration = variance(Power_Brake_df_hp)
#CV variation coefficient = Standard deviation / mean. If CV < 1 indicates a low variation, however for non-normal distribution the standard deviation does not mean anything, also the CV
#It just says how much your data is spread from the mean...but does not say if it is wrong!
CV_Brake_Power = std_dev_Astar_brake / Mean_Astar_brake_power
...
remove_Outliers_velocity_df_IQR_method = Joined_velocity_xy_df[~((Joined_velocity_xy_df < (Q1_v - 1.5 * IQR_v)) |(Joined_accel_pose_df > (Q3_v + 1.5 * IQR_v))).any(axis=1)]
...
Final_Power_Astar_mean_plus_stdev = Mean_Power_Consumption + standard_deviation_Power_Consumption
