...
#Get velocity ---ATTENTION --this csv contains less rows, I will just use to calculate the average speed
velocity_xy_forward_array = df_velocity['Speed_mps']
...
#REmove the opposite acceleration that brake the car. The motor is not generating Horsepower (Hp) for braking and positive values will prejudice take the average or median of the acceleration waypoints 
drop_brake_opposite_acceleration_x_list = np.array( [num for num in final_list_delta_linear_accel_x if num>=0])
final_value_average_accel_x = (np.nansum(drop_brake_opposite_acceleration_x_list)) / len(drop_brake_opposite_acceleration_x_list)
...
#Actually the code below does not drop because of matching purposes it replace negative values by 0
delta_velocity_drop_negative_x_values_List =  np.clip(delta_velocity_x_list,0,1000)
delta_velocity_x_value_average = np.nansum(delta_velocity_drop_negative_x_values_List) /len(delta_velocity_drop_negative_x_values_List)
...
drop_brake_opposite_acceleration_y_list = np.array( [num for num in final_list_delta_linear_accel_x if num>=0])
final_value_average_accel_y = (np.nansum(drop_brake_opposite_acceleration_y_list)) / len(drop_brake_opposite_acceleration_y_list)
...
delta_velocity_drop_negative_y_values_List =  np.clip(delta_velocity_y_list,0,1000)
delta_velocity_y_value_average = np.nansum(delta_velocity_drop_negative_y_values_List) /len(delta_velocity_drop_negative_y_values_List)
...
linear_accel_pow_x_list = [e **2 for e in drop_brake_opposite_acceleration_x_list]
linear_accel_pow_y_list = [f **2 for f in drop_brake_opposite_acceleration_y_list]
...
linear_vel_pow_x_value = delta_velocity_drop_negative_x_values_List **2
linear_vel_pow_y_value = delta_velocity_drop_negative_y_values_List **2
...
linear_vel_xy_average = np.nansum(linear_velocity_xy_resultant_module_list)
linear_vel_xy_median = np.median(linear_velocity_xy_resultant_module_list)

#THE CALCULATIONS ABOVE ARE FROM DELTA VEL. THE Eq BELOW IS FROM AVERAGE VELOCITY (NOT DELTA, I am using other dataframe)

velocity_linear_xy_forward_Average = np.nansum(velocity_xy_forward_array) / len(velocity_xy_forward_array)
velocity_linear_xy_forward_Median = np.median(velocity_xy_forward_array)
...
Total_Acceleration_Linear_plus_Rotational_average = linear_accel_xy_average + Accel_xy_resultant_components_average
...
Ft_Value_Average = M * Total_Acceleration_Linear_plus_Rotational_average #(linear_accel_xy_average + Total_Accel_Lin_Rot_Average)

#-----------------------------------------------------------------------------------------------------------------
#TASK 7: FIND THE TOTAL VELOCITY OF THE VEHICLE = VT = V_linear_X + Vrotational_converted_in_x_y_components

Vel_Linear_xy_total = velocity_linear_xy_forward_Average #linear_velocity_xy_resultant_module_list
...
Power_Linear_Vel_x_List = Vrot_linear_x_component_average **2
Power_Linear_Vel_y_List = Vrot_linear_y_component_average **2
