...
for p_x in range(len(pose_x_array)-1):
    ...
    delta_pose_x_value = pose_x_array.iloc[p_x+1] - pose_x_array.iloc[p_x]
...
for p_y in range(len(pose_y_array)-1):
    ...
    delta_pose_y_value = pose_y_array.iloc[p_y+1] - pose_y_array.iloc[p_y]
...
Positive_acceleration_df = Filtered_Parameters_Df[(Filtered_Parameters_Df[['Acceleration X']] > 0).all(1)]
Brake_Acceleration_df = Filtered_Parameters_Df[(Filtered_Parameters_Df[['Acceleration X']] < 0).all(1)]

Acceleration_xy_df = Positive_acceleration_df[['Acceleration X', 'Acceleration Y']]
...
Action_Acceleration_df = Force_Acceleration_df * Positive_acceleration_df['Delta Pose XY'] 


#Taks 10: Calculate Power in Hp

Power_Acceleration_df_watts = Action_Acceleration_df / Positive_acceleration_df['Delta Time']
